...
! To find upper limit for M(=max) (using e explicitly here but)
  max = k * log(10.0)    ! max is an INTEGER, so automatically rounded off.

! main loop
  DO m = max, 2, -1      ! reverse order (-1), from the most inner ()
    r = 1
    DO n = 1, k
      j = 10*r + keta(n)
